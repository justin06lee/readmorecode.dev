...
export async function generatePuzzle(
  ...
  const singleRepo = options?.repos && options.repos.length === 1;
  const maxRepoAttempts = singleRepo ? 1 : 5;
  let lastError = `All ${maxRepoAttempts} repo attempts failed.`;
  ...
  for (let repoAttempt = 0; repoAttempt < maxRepoAttempts; repoAttempt++) {
    ...
    const fileList = tree.filter((f) => isPathForLanguage(f.path, targetLanguage));
    if (fileList.length === 0) {
      log(`  No ${targetLanguage} files in this repo, next repo`);
      continue;
    }
    shuffle(fileList, rng);
    log(`  ${fileList.length} ${targetLanguage} files; will try up to ${MAX_FILE_TRIES_PER_REPO} from this repo`);
    ...
    for (let fileTry = 0; fileTry < Math.min(MAX_FILE_TRIES_PER_REPO, fileList.length); fileTry++) {
      ...
      if (!fileInfo) {
        log(`    Fetch failed (404 or too large), next file`);
        continue;
      }
      if (fileInfo.content.length < 100) {
        log(`    Too short (${fileInfo.content.length} bytes), next file`);
        continue;
      }
      ...
      let parsed: LLMPuzzleRaw | null = null;
      for (let llmTry = 0; llmTry < MAX_LLM_RETRIES; llmTry++) {
        try {
          ...
          parsed = obj ? objToLLMPuzzleRaw(obj) : null;
          if (parsed) break;
          if (llmTry < MAX_LLM_RETRIES - 1) {
            log(`    Invalid JSON (attempt ${llmTry + 1}/${MAX_LLM_RETRIES}), retrying...`);
          } else {
            log(`    Invalid JSON after ${MAX_LLM_RETRIES} attempts, next file`);
          }
        } catch (err) {
